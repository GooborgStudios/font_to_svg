...
namespace font2svg {
	...
	*/
	std::string do_outline(std::vector<FT_Vector> points,
		...
		svg.str("");
		svg << "\n\n  <!-- draw actual outline using lines and Bezier "
			   "curves-->";
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			debug << "moving to first pt " << points[offset].x << ","
				  << points[offset].y << "\n";
			svg << "\n M " << points[contour_starti].x << ","
				<< points[contour_starti].y << "\n";
			debug << "listing pts: [this pt index][isctrl] <next pt "
					 "index><isctrl> "
					 "[x,y] <nx,ny>\n";
			for (int j = 0; j < npts; j++) {
				...
				if (this_isctl && next_isctl) {
					debug << " two adjacent ctl pts. adding point halfway "
							 "between "
						  << thisi << " and " << nexti << ":";
					...
					debug << " [" << x << "," << y << "]\n";
					if (j == 0) {
						debug << "first pt in contour was ctrl pt. moving to "
								 "non-ctrl pt\n";
						svg << " M " << x << "," << y << "\n";
					}
				}
	...
	class glyph {
	  public:
		...
		void init(std::string unicode_s) {
			...
			debug << " (decimal: " << codepoint << " hex: 0x" << std::hex
				  << codepoint << std::dec << ")";
			...
			debug << "\nGlyph Width: " << gm.width << " Height: " << gm.height
		...
		std::string axes() {
			...
			tmp << "\n <path stroke='blue' stroke-dasharray='5,5' d='"
				<< " M" << -bbwidth << "," << 0 << " L" << bbwidth << "," << 0
				<< " M" << 0 << "," << -bbheight << " L" << 0 << "," << bbheight
				<< " '/>";
			return tmp.str();
		}
		...
		std::string pointlines() {
			...
			for (int i = 0; i < ftoutline.n_points - 1; i++) {
				...
				tmp << " L " << ftpoints[(i + 1) % ftoutline.n_points].x << ","
					<< ftpoints[(i + 1) % ftoutline.n_points].y;
				tmp << "\n  '/>";
			}
			return tmp.str();
		}
		...
		std::string outline() {
			std::vector<FT_Vector> pointsv(ftpoints,
			                               ftpoints + ftoutline.n_points);
			std::vector<char> tagsv(tags, tags + ftoutline.n_points);
			std::vector<short> contoursv(contours,
			                             contours + ftoutline.n_contours);
